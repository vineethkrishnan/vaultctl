...
func (e *ScriptExecutor) Execute(ctx context.Context) <-chan Event {
	...
	go func() {
		...
		ch <- Event{Type: EventLog, Msg: fmt.Sprintf("Running upgrade script: %s", e.ScriptPath)}

		cmd := exec.CommandContext(ctx, e.ScriptPath) //nolint:gosec // path is operator-configured, not user-supplied
		stdout, err := cmd.StdoutPipe()
		if err != nil {
			ch <- Event{Type: EventError, Msg: fmt.Sprintf("pipe: %v", err)}
			return
		}
		cmd.Stderr = cmd.Stdout // merge stderr into same pipe

		if err := cmd.Start(); err != nil {
			ch <- Event{Type: EventError, Msg: fmt.Sprintf("start script: %v", err)}
			return
		}

		scanner := bufio.NewScanner(stdout)
		for scanner.Scan() {
			ch <- Event{Type: EventLog, Msg: scanner.Text()}
		}

		if err := cmd.Wait(); err != nil {
			ch <- Event{Type: EventError, Msg: fmt.Sprintf("script exited with error: %v", err)}
			return
		}

		ch <- Event{Type: EventRestarting, Msg: "Script completed. Server is restarting."}
	}()
	return ch
}
