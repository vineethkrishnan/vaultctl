...
// ListDue returns users whose digest is due, joined to their email.
func (r *DigestPrefsRepo) ListDue(ctx context.Context, now time.Time) ([]ports.DueDigest, error) {
	rows, err := r.Pool.Query(ctx, `
		SELECT p.user_id, u.email, p.frequency, p.last_run_at
		FROM user_digest_prefs p JOIN users u ON u.id = p.user_id
		WHERE p.frequency <> 'off' AND p.next_run_at IS NOT NULL AND p.next_run_at <= $1
	`, now)
	if err != nil {
		return nil, fmt.Errorf("list due digests: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		var uid string
		if err := rows.Scan(&uid, &d.Email, &d.Frequency, &d.LastRunAt); err != nil {
...
// MarkRun records a completed send and schedules the next.
func (r *DigestPrefsRepo) MarkRun(ctx context.Context, userID user.ID, lastRunAt time.Time, nextRunAt *time.Time) error {
	_, err := r.Pool.Exec(ctx, `
		UPDATE user_digest_prefs SET last_run_at = $2, next_run_at = $3, updated_at = $2 WHERE user_id = $1
	`, string(userID), lastRunAt, nextRunAt)
	if err != nil {
		return fmt.Errorf("mark digest run: %w", err)
	}
	return nil
}
