...
**Yes.** vaultctl ships as a single static Go binary. You can download it from the [releases page](https://github.com/vineethkrishnan/vaultctl/releases) and run it directly:
...
curl -fsSL https://vaultctl.dev/install.sh | bash -s -- --no-docker
```

You will need to provide your own PostgreSQL instance and TLS termination (reverse proxy).
