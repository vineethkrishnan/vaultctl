...
The icon also appears while the vault is **locked**, once the extension has been set up (controlled by **Show icon when locked** in Settings). Clicking it opens the unlock prompt -- the toolbar popup where the browser allows it, otherwise a small extension window -- with your server and account already in place, so you only enter your master password (or use Touch ID). After unlocking, the account picker reopens on the field you clicked so you can fill it right away.
...
| **Show icon when locked** | On | Show the field icon even while the vault is locked; clicking it opens the unlock prompt. |
...
| **Auto-lock** | 15 min | Lock the extension after this much inactivity (1 / 5 / 15 / 30 / 60 minutes, or Never). |
