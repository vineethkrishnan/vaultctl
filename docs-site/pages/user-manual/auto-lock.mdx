...
On the login screen (web and the browser extension) you can tick **Remember me on this device**. When set, your email address is stored locally on that device and prefilled on your next visit, taking you straight to the master-password step so unlocking only needs your password.

Only the **email** is remembered. Your master password and keys are never stored, so this is a convenience, not a reduction in security. Choosing **change** (web) or **Use a different account** (extension) forgets the saved email.
